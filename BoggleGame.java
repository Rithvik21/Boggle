...
public class BoggleGame extends JFrame {
	...
	public BoggleGame(int rows, int cols) {
        ...
        Scanner in = null;
        try { in = new Scanner(new File("dictionary-shakespeare.txt")); } catch (FileNotFoundException f) {}
        shakespeareDictionary = new HashSet<String>();
        while (in.hasNext())
            shakespeareDictionary.add(in.next());

        // all words in shakespeare
        try { in = new Scanner(new File("dictionary-nursery.txt")); } catch (FileNotFoundException f) {}
        nurseryDictionary = new HashSet<String>();
        while (in.hasNext())
            nurseryDictionary.add(in.next());

        // about 20K common words
        try { in = new Scanner(new File("dictionary-common.txt")); } catch (FileNotFoundException f) {}
        commonDictionary = new HashSet<String>();
        while (in.hasNext())
            commonDictionary.add(in.next());

        // all words in Algorithms 4/e
        try { in = new Scanner(new File("dictionary-algs4.txt")); } catch (FileNotFoundException f) {}
        algs4Dictionary = new HashSet<String>();
        while (in.hasNext())
            algs4Dictionary.add(in.next());

        // create the Boggle solver with the big dictionary
        solver = new BoggleSolver("dictionary-yawl.txt");
